...
```
Every node created for a goal should be linked. `review_goal` uses linked nodes to assess coverage — unlinked knowledge is invisible to coverage evaluation.
...
2. `manage_goal(operation: "get_goal", goal_id: "<top-priority-id>")`
